...
* Embedding a useful tool by itself:
...
* Input for PCA:
  Data matrix X, columns are features adjusted to have zero emperical mean, rows are experiments
* PCA in terms of eigenvectors:
  Equivalent to eigenvector decompostion of (transposed data matrix)*(data matrix)
* 
...
#  LocalWords:  DDNRegressor memoizing Adagrad Softmax len NxM PCA
