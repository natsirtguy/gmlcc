...
* Parsing function for single TFRecordDataset record:
  Create dictionary with feature/label names as keys, tf feature types as values, then call tf.parse_single_example(record, dict), then return tuple with dictionary of feature names and values, labels
...
* Save dataset from website using keras:
...
#  LocalWords:  TFRecord dict DNNClassifier
