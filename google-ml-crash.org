...
* Repeat=None in tf.data.Dataset means:
  That the data will repeat indefinitely, must set repeat=1 for predicitions
* Feature engineering:
...
* Examples of human biases in data:
  ORSOCA: overgeneralization, reporting bias, selection bias, out-group homogeneity bias, confirmation bias, automation bias
