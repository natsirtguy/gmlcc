...
* probabilities in linear_classifier.predict() elements:
  Array of probabilities of the different classes, probabilities[0] is prob that it is false for Boolean, probabilities[1] is equivalent to logistic
* classes in linear_regressor.predict() elements:
  Array with the most likely class for the given prediction
* class_id in linear_regressor.predict() elements:
